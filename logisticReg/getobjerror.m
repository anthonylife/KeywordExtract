...
for i=1:length(docinfo)
    ...
    sum_expvalue = getsum_expvalue(docinfo(i).features, model);
    docinfo(i).pv = getnodevalue(docinfo(i).features,...
                    1:length(docinfo(i).labels), model.w);
    for j=1:length(pos_ins),
        totalerror=totalerror+sum(1/(1+exp(-(docinfo(i).pv(neg_ins)...
                -docinfo(i).pv(pos_ins(j))))));
    end
end
